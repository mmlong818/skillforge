...
#### 0. 是否应该做成 Skill（资格判定）
先判断这个需求是否值得做成 Skill，避免为一次性任务过度封装。对照以下规则给出结论：
- **直接回答即可**：解释概念、总结文档、翻译、单纯头脑风暴、一次性问题 → 不需要 Skill
- **写成文档**：知识库/wiki、无需 agent 执行的可复用资料、政策备忘 → 用文档而非 Skill
- **写成脚本**：确定性任务、不存在触发歧义、用户要的是工具而非可路由能力 → 用脚本
- **做成 Skill（需同时满足）**：任务会重复出现 + 需要被自动发现/触发 + 需要明确边界 + 有可复用的输出契约
- **拿不准**：先做成 Scaffold（探索版），出现真实复用后再升级

输出「资格结论」：判定为「适合做成 Skill」或「建议改用 直接回答 / 文档 / 脚本」，并说明理由。
- 红旗：「以后可能有用」属于臆测，不构成理由；等真实用例出现再做。
- 若判定不适合做成 Skill，明确标注并给出替代方案；如用户仍坚持，则按需求继续后续步骤。
...
#### 第一步：构造触发测试用例（trigger cases）

description 决定 Skill 是否被正确触发，因此先构造测试集，用它来验证 description，而不是仅凭直觉。列出三类用例（中英混合，贴近真实用户口吻）：

- **应触发（should_trigger，5-8 条）**：用户真实会说、且必须命中本 Skill 的请求 —— 用于测召回
- **不应触发（should_not，3-5 条）**：明显超出职责范围的请求 —— 用于测精确
- **相邻易混（near_neighbor，3-5 条）**：看起来相似但职责不同的请求 —— 最容易误触发，重点考察

再提炼 3-6 个**排除关键词**（negative patterns，如「只要总结」「翻译一下」「一次性」「头脑风暴」等，命中即不应触发的信号）。

#### 第二步：生成 3 个候选 description
...
对每个候选 description，逐条走查第一步的测试集：

| 候选 | should_trigger 命中 | should_not 误触发 | near_neighbor 误触发 | 结论 |
|------|--------------------|-------------------|----------------------|------|
| A    |                    |                   |                      |      |
| B    |                    |                   |                      |      |
| C    |                    |                   |                      |      |

判定规则：优先选「should_trigger 命中最多、且 should_not / near_neighbor 误触发最少」的候选，**实测结果优先于第二步的自评分**。若最佳候选仍在某条 near_neighbor 上误触发，先收紧 description（补充排除表述）再定稿。

#### 第四步：输出最终 YAML frontmatter

选择实测最优的候选，输出最终版本。在 YAML frontmatter 之前，用普通文本保留上面的触发测试集（供后续回归复用；若架构决策中规划了 references/trigger-cases.md，则一并写入该文件）。
